...
class ram():
	...
	def imprime_archivos(self):
		for i in range(len(self.nombres_archivos)):
			print self.nombres_archivos[i]
			print " en bloque " + str(i) + " de RAM"
	...
	def leer_archivo(self,name_archivo):
		...
		if name_archivo in self.nombres_archivos:
			posicion = self.nombres_archivos.index(name_archivo) #obtenemos el id que representa a ese archivo
			#AHORA debo verificar que el archivo este en la RAM, sino lo iremos a buscar
			encontrado = False

			for x in xrange(0,20):
				if posicion == self.bloques_memoria[x][0]: #si esta dentro de la memoria RAM, sino, busco el disco y copio a RAM
					encontrado = True
			...
			if encontrado == True:
				...
				partes = []
				for i in range(0,20):
					if self.bloques_memoria[i][0] == posicion:
						partes.append([i,self.bloques_memoria[i][1]]) #Guardo el archivo que debo leer y el offset
