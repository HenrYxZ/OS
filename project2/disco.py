	...
	def Agregar_Bloques(self, nombres_BloquesRam):
		for name in nombres_BloquesRam:
			...
			if(tamano_archivo > 512):    #ocupamos 2 bloques
				...
				bloques_disco[posicion_actual] = 1  #Marcamos los bloques a utilizar
				bloques_disco[posicion_actual+1] = 1
				archivo1 = open("disco/" +str(posicion_actual) + ".txt", "w")
				archivo1.write(bloque1)   #Escribimos el bloque 
				archivo1.close()
				archivo2 = open("disco/" +str(posicion_actual+1) + ".txt", "w")
				...
				bloques_disco[posicion_actual]=1  #marcamos el bloque
				archivo1 = open("disco/" +str(posicion_actual) + ".txt", "w")
